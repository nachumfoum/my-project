                      ...
                      <div class="card-body">
                        ...
                        <table class='table table-hover table-bordered mt-1' id="post-table">
                          ...
                          <tbody>
                            <?php
                              $no =  1;
                              $app = mysqli_query($con,"select a.*, p.*, s.* from application a JOIN post p ON a.postId = p.postID  JOIN applicant s ON a.applicantEmail = s.email where p.officeName = '".$profile['officeName']."' order by a.gpa desc; ");
